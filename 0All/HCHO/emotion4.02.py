...
def SelectData(dataName):
    try:
        sql_command_select = "SELECT photoId,url FROM public.FaceTable_{0} WHERE faceNum is null limit 1;".format(
            dataName)
        ...
        sql_command_update = "UPDATE public.FaceTable_{0} SET faceNum={2},attributes='{3}' WHERE photoId={1}".format(
            dataName, data[0], i, faceIn)
